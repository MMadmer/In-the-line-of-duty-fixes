...
* The courier suit no longer falls apart within minutes: its immunity section carried 1.00 for all nine hit types (every other suit carries hundredths, and none wears from radiation or psi at all), so every hit took a third of its condition. It now wears like the stalker suit it is cut from. A suit worn down before the update, to 0% included, is restored to 100% once: at the first run of 1.0.11 if it is worn or in the rucksack, or when it is taken out of a stash later; from then on it wears as it should and is not repaired again.
